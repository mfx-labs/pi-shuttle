...
function shuttleFiles(version = '0.1.2', body = 'console.log("fixture pi-shuttle");'): Record<string, string> {
  return {
...
function writeShuttlePackage(root: string, version = '0.1.2', body = 'console.log("fixture pi-shuttle");'): void {
...
function seedActiveLatest(layout: ReturnType<typeof resolveLayout>, source = SOURCE, body = 'console.log("fixture pi-shuttle");'): string {
  const target = join(layout.packagesDir, piShuttlePackageDirName('0.1.2', source));
  writeShuttlePackage(target, '0.1.2', body);
...
function seedMatchingActiveLatest(layout: ReturnType<typeof resolveLayout>, source = SOURCE): string {
  const target = join(layout.packagesDir, piShuttlePackageDirName('0.1.2', source));
  cpSync(join(layout.packagesDir, 'pi-shuttle@0.1.2'), target, { recursive: true });
  return activateLatestCommand(layout, target);
}
...
async function fixtureArtifacts(env: string) {
  ...
  const shuttle = await buildTarball(env, shuttleFiles(), 'pi-shuttle-0.1.2.tgz');
...
test('Latest source changes require source-aware consent; an exact source is already installed', async () => {
  ...
  try {
    ...
    assert.deepEqual(exact, { kind: 'ALREADY_INSTALLED', version: '0.1.2' });
...
test('multiple retained pi-shuttle versions are INCOMPLETE evidence, not ambiguity', async () => {
  ...
  try {
    ...
    assert.equal(existsSync(join(layout.packagesDir, 'pi-shuttle@0.1.2')), true);
  } finally {
    cleanupEnv(env);
  }
});
...
test('an inactive incomplete exact Latest destination is removed and recreated only after consent', async () => {
  ...
  try {
    ...
    for (const [name, content] of Object.entries(shuttleFiles('0.1.2', 'console.log("incomplete bytes");'))) writeFileSync(join(target, name), content, { mode: 0o700 });
    const state = classify(env);
    assert.equal(state.kind, 'INCOMPLETE');
...
test('FINAL Latest receipt publication happens after activation and rolls activation back if atomic publication fails', async () => {
  ...
  try {
    ...
    assert.equal(existsSync(join(layout.packagesDir, piShuttlePackageDirName('0.1.2', SOURCE))), false, 'attempt package rolls back');
  } finally {
    cleanupEnv(env);
  }
});
...
test('exact physical-machine fixture cleans/reinstalls without touching projects, config, stores, Gateway, or pi-guard', async () => {
  ...
  try {
    ...
    const layout = resolveLayout(env);
    const final = JSON.parse(readFileSync(layout.installReceiptPath, 'utf8')) as Record<string, unknown>;
    const retained = join(layout.packagesDir, 'pi-shuttle@0.1.2');
    ...
    assert.equal(existsSync(retained), true, 'retained 0.1.2 package is retained');
    const latest = join(layout.packagesDir, piShuttlePackageDirName('0.1.2', PHYSICAL_SOURCE));
