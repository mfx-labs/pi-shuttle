 ...
 * prepare-fixtures.sh lane selection and dual-macOS-boundary verification.
 ...
 * lane-selected expected commit), and the macOS tarball boundary is
...
const ARM64_ADDON_SHA256 = 'f43705523b6859dc33283b75391e0ebf7cddf0779a877ee2edf7767152a946be';
...
test('lane selection: linux selects the historical Gateway commit', () => {
  ...
  try {
    ...
    assert.ok(!run.output.includes(MACOS_COMMIT), 'linux lane must never pin the macOS commit');
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});

test('lane selection: darwin-arm64 selects the shared macOS commit', () => {
  const root = join('/tmp', `ps6b-lane-test-${process.pid}-${Date.now()}`);
  mkdirSync(root, { recursive: true });
  try {
    makeRepo(root, 'gateway');
    ...
    assert.equal(run.code, 1, 'synthetic HEAD must fail the exact-pin check');
    assert.ok(run.output.includes(`expected ${MACOS_COMMIT}`), `arm64 lane must pin the macOS commit:\n${run.output}`);
    assert.ok(!run.output.includes(`expected ${HISTORICAL_COMMIT}`), 'arm64 lane must never pin the historical commit');
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
...
test('lane selection: darwin-x86_64 selects the macOS fork commit', () => {
  ...
  try {
    ...
    assert.equal(run.code, 1, 'synthetic HEAD must fail the exact-pin check');
    assert.ok(run.output.includes(`expected ${MACOS_COMMIT}`), `Intel lane must pin the macOS commit:\n${run.output}`);
...
test('script statics: the three lane identities and dual-macOS boundary are committed exactly', () => {
  ...
  assert.ok(text.includes(`GATEWAY_COMMIT_MACOS="${MACOS_COMMIT}"`), 'shared macOS pin literal present');
  ...
  // Dual-architecture tarball boundary: required entries present, debris absent.
  for (const entry of ['package/package.json', 'package/native/index.mjs', 'package/native/darwin-x64/gateway_fs.node', 'package/native/darwin-arm64/gateway_fs.node', '^package/dist/']) {
    assert.ok(text.includes(entry), `macOS required boundary entry committed: ${entry}`);
  }
  for (const entry of ['package/native/src', 'package/native/build', 'package/native/test']) {
    assert.ok(text.includes(entry), `macOS forbidden boundary entry committed: ${entry}`);
  }
  assert.ok(/if \[ "\$LANE" = "\$DARWIN_ARM64_HOST_LANE" \] \|\| \[ "\$LANE" = "\$DARWIN_X86_64_HOST_LANE" \][\s\S]{0,400}?GATEWAY_COMMIT_EFFECTIVE="\$GATEWAY_COMMIT_MACOS"/.test(text), 'both Darwin lanes must assign the shared macOS pin');
  ...
  // PGM-DIST-2: both addons are TRACKED in the pinned clean tree with exact
  // digests; neither may be copied from the operator checkout.
  ...
  assert.ok(text.includes('git -C "$WORK/gateway" ls-files --error-unmatch -- "$ADDON_REL"'), 'script must prove each addon is TRACKED in the scratch clone');
  assert.ok(text.includes(INTEL_ADDON_SHA256), 'script must pin the accepted x64 addon digest');
  assert.ok(text.includes(ARM64_ADDON_SHA256), 'script must pin the arm64 candidate digest');
  assert.ok(text.includes('tar -xOf "$TARBALL" "$ADDON_REL"'), 'script must verify addon bytes after packing');
  assert.ok(text.includes('does not add an arm64 native-load/acceptance assertion'), 'arm64 packaging must not claim native-load acceptance');
});
