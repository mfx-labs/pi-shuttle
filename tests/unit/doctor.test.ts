...
test('doctor: Latest receipt verifies its semantic version, exact source slot, command target, and package bytes', async () => {
  ...
  try {
    ...
    const root = join(layout.packagesDir, piShuttlePackageDirName('0.1.2', sourceIdentity));
    mkdirSync(join(root, 'dist'), { recursive: true, mode: 0o700 });
    mkdirSync(layout.binDir, { recursive: true, mode: 0o700 });
    writeFileSync(join(root, 'package.json'), JSON.stringify({ name: 'pi-shuttle', version: '0.1.2', bin: { 'pi-shuttle': './dist/cli.js' } }));
    ...
    const wrongPath = join(layout.packagesDir, piShuttlePackageDirName('0.1.2', `mfx-labs/pi-shuttle@${'c'.repeat(40)}`));
    ...
    const stable = join(layout.packagesDir, 'pi-shuttle@0.1.2');
    mkdirSync(join(stable, 'dist'), { recursive: true, mode: 0o700 });
    writeFileSync(join(stable, 'package.json'), JSON.stringify({ name: 'pi-shuttle', version: '0.1.2', bin: { 'pi-shuttle': './dist/cli.js' } }));
