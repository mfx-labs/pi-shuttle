/** 
 * HISTORICAL previous-generation installer entry (FRESH-INSTALL Slice).
 *
 * This module is the unmodified previous-generation local-lane installer
 * entry, retained ONLY as a test-support harness for the historical
 * installer suites (installer-flow etc.). It is COMPLETELY UNREACHABLE
 * from the production install authority: the production entries
 * (`dist/installer/main.js`, `dist/installer/release/bootstrap.js`) route
 * exclusively through the manifest-native orchestrator
 * (`src/manifest-native/install.ts`) and never import this module. The
 * previous-generation install core (`runInstall`) and its concrete
 * release pins remain historical code tested through this harness.
 *
